...
class Acquire:
    ...
    def handle_query(self, request, boardMatrix,place=True):
        returnMsg = ''
        ...
        res = self.handle_singleton(request, boardMatrix,place)
        if type(res) == dict and "singleton" in res:
            returnMsg = res
        else:
            merging_res = self.handle_merging(request, boardMatrix,place)
            if type(merging_res) == dict and "acquirer" in merging_res:      
                returnMsg = merging_res
            else:
                growing_resonse = self.handle_growing(request, boardMatrix,place)
                if type(growing_resonse) == dict and "growing" in growing_resonse: 
                    returnMsg = growing_resonse
                else:
                    if "hotel" in request.keys():
                        founding_response = self.handle_founding(request, boardMatrix,place)
    ...
    def handle_singleton(self, request, boardMatrix,place=True):
        ...
        if singleTonFlag:
            ...
            return {"type":"singleton","singleton": None}
        else:
            return Error("Other Operation")

    def handle_growing(self, request, boardMatrix,place=True):
        row = ord(request["row"]) - 65
        col = int(request["column"]) - 1
        if boardMatrix[row][col] != "0":
            return Error("A Tile cannot be placed at the desired location")
        ...
        return {"type":"growing","growing": hotel,"hotel":hotel}

    def handle_founding(self, request, boardMatrix,place=True):
        row = ord(request["row"]) - 65
        col = int(request["column"]) - 1
        ...
        return {"type":"founding","founding": hotel_label,"hotel":hotel_label}

    def handle_merging(self, request, boardMatrix,place=True):
        row = ord(request["row"]) - 65
        col = int(request["column"]) - 1
        merging_flag = True
        if boardMatrix[row][col] != "0":
        ...
        if merging_flag:
            ...
            return {"type":"merging","acquirer": acquirer, "acquired": acquired, "acquired_hotels_dict" : acquired_hotels}
        else:
            return Error("Other Operation")
